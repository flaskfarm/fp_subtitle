...
class SupportSmi2srt(object):
    ...
    @classmethod
    def start(cls, work_path, remake=False, no_remove_smi=False, no_append_ko=False, no_change_ko_srt=False, fail_move_path='', not_smi_move_path=''):
        cls.remake = remake
        cls.no_remove_smi = no_remove_smi
        cls.no_append_ko = no_append_ko
        cls.no_change_ko_srt = no_change_ko_srt
        cls.fail_move_path = fail_move_path
        cls.not_smi_move_path = not_smi_move_path
        cls.result_list = {}
        cls.result_list['option'] = {'work_path':work_path, 'remake':remake, 'no_remove_smi':no_remove_smi, 'no_append_ko':no_append_ko, 'no_change_ko_srt':no_change_ko_srt, 'fail_move_path':fail_move_path, 'not_smi_move_path':not_smi_move_path}
        cls.result_list['list'] = []
        try:
            work_path = work_path
            if os.path.isdir(work_path):
                cls.convert_directory(work_path)
            else:
                parent_path = os.path.dirname(work_path)
                lists = [os.path.basename(work_path)]
                cls.convert_directory(parent_path, lists)
    ...
    @classmethod
    def convert_directory(cls, work_path, lists=None):
                ...
                try:
                    ...
                    if os.path.isdir(eachfile):
                        ...
                        if eachfile[-4:].lower() == '.smi':
                            rndx = eachfile.rfind('.')
                            if SupportSmi2srt.no_append_ko or eachfile.lower().endswith('.kor.smi') or eachfile.lower().endswith('.ko.smi'):
                                srt_file = '%s.srt' % eachfile[0:rndx]
                            else:
                                srt_file = '%s.ko.srt' % eachfile[0:rndx]
                            if os.path.exists(srt_file):
                                if SupportSmi2srt.remake:
                                    #logger.debug('remake is true..')
                                    pass
                                else:
                                    #logger.debug('remake is false..')
                                    # 2021-12-19
                                    if not SupportSmi2srt.no_remove_smi:
                            ...
                            ret = SupportSmi2srt.convert_one_file_logic(eachfile, srt_file)
                            logger.info("Convert result : %s", ret)
                            if ret['ret'] == "success":
                                if not SupportSmi2srt.no_remove_smi:
                                    logger.debug("remove smi")
                                    os.remove(eachfile)
                            elif ret['ret'] == "not_smi":
                                if SupportSmi2srt.not_smi_move_path:
                                    target = os.path.join(SupportSmi2srt.not_smi_move_path, item)
                                    if eachfile != target:
                                        shutil.move(eachfile, target)
                                        ret['move_path'] = target
                            elif ret['ret'] == "fail":
                                if SupportSmi2srt.fail_move_path:
                                    target = os.path.join(SupportSmi2srt.fail_move_path, item)
                                    if eachfile != target:
                                        shutil.move(eachfile, target)
                                        ret['move_path'] = target
                            ...
                            SupportSmi2srt.result_list['list'].append(ret)
                            ...
                            if not SupportSmi2srt.no_change_ko_srt:
    ...
    @classmethod
    def predict_encoding(cls, file_path, n_lines=100):
    ...
    @classmethod
    def convert_one_file_logic(cls, smi_file, srt_file):
        try:
            ret = {'smi_file':smi_file}
            if not os.path.exists(smi_file):
                return {'ret':'fail'}

            encoding = SupportSmi2srt.predict_encoding(smi_file).lower()
            ...
            data = SupportSmi2srt.demuxSMI(smi_sgml)
            ...
            for lang, smi_sgml in data.items():
                ...
                try:
                    ...
                    if fndx < 0:
                        ret['ret'] = SupportSmi2srt.process_not_sync_tag(smi_sgml)
                        return ret
    ...
    @classmethod
    def process_not_sync_tag(cls, text):
        try:
...
if __name__ == '__main__':
    ...
    ret = SupportSmi2srt.start(args.work_path, remake=args.remake, no_remove_smi=args.no_remove_smi, no_append_ko=args.no_append_ko, no_change_ko_srt=args.no_change_ko_srt, fail_move_path=args.fail_move_path)
